...
def compose_lidar2img(ego2global_translation_curr,
    ...
    sensor2global_rt = np.eye(4)

    lidar2ego_rt[:3, :3] = lidar2ego_rotation_curr
    lidar2ego_rt[:3, 3] = lidar2ego_translation_curr
    ego2global_rt[:3, :3] = ego2global_rotation_curr
    ego2global_rt[:3, 3] = ego2global_translation_curr
    sensor2global_rt[:3, :3] = sensor2global_rotation_past
    sensor2global_rt[:3, 3] = sensor2global_translation_past
    lidar2cam_rt = np.linalg.inv(sensor2global_rt) @ ego2global_rt @ lidar2ego_rt

    viewpad = np.eye(4)
    viewpad[:cam_intrinsic_past.shape[0], :cam_intrinsic_past.shape[1]] = cam_intrinsic_past
    lidar2img = (viewpad @ lidar2cam_rt).astype(np.float32)

    return lidar2img


def get_sensor_transforms(rotation, translation):
    mat = np.eye(4)
    mat[:3, :3] = rotation
    mat[:3, 3] = translation
    return mat
...
class LoadMultiViewImageFromFiles(object):
    ...
    def __call__(self, results):
        ...
        """
        filename = [CAM['data_path'] for CAM in results['curr']['cams'].values()]
        ...
        results['img_timestamp'] = [CAM['timestamp']/1e6 for CAM in results['curr']['cams'].values()]
        ...
        results['img_norm_cfg'] = dict(
            ...
            to_rgb=False)
        lidar2img_rts = []
        for _, cam_info in results['curr']['cams'].items():
            ...
            lidar2img_rts.append(viewpad @ np.linalg.inv(cam2lidar_rt))
        lidar2ego = np.eye(4)
        lidar2ego[:3,:3] = Quaternion(results['curr']['lidar2ego_rotation']).rotation_matrix
        lidar2ego[:3,3] = np.array(results['curr']['lidar2ego_translation'])
        results.update(dict(
...
class LoadMultiViewImageFromMultiSweeps:
    ...
    def load_offline(self, results):
        ...
        #         choices = [(k + 1) * interval - 1 for k in range(self.sweeps_num)]
        choices = range(self.sweeps_num)
        for idx in sorted(list(choices)):
            # sweep_idx = min(idx, len(results['cam_sweeps']['prev']) - 1)
            sweep = results['adjacent'][idx+1]['cams']
            ...
            for sensor in cam_types:
                ...
                # cam2ego_view = np.eye(4)
                # cam2ego_view[:3,:3] = Quaternion(sweep[sensor]['sensor2ego_rotation']).rotation_matrix
                # cam2ego_view[:3,3] =

                cam2global_r = Quaternion(sweep[sensor]['ego2global_rotation']).rotation_matrix @  Quaternion(sweep[sensor]['sensor2ego_rotation']).rotation_matrix
                cam2global_t = (Quaternion(sweep[sensor]['ego2global_rotation']).rotation_matrix @ np.array(sweep[sensor]['sensor2ego_translation']) +
                                np.array(sweep[sensor]['ego2global_translation']))

                results['lidar2img'].append(compose_lidar2img(
                    results['curr']['ego2global_translation'],
                    Quaternion(results['curr']['ego2global_rotation']).rotation_matrix,
                    results['curr']['lidar2ego_translation'],
                    Quaternion(results['curr']['lidar2ego_rotation']).rotation_matrix,
                    cam2global_t,
                    cam2global_r,
