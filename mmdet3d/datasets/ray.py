...
def pts2ray(coor, label_depth, label_seg, label_img, c2w, cam_intrinsic):
...
def generate_rays(coors, label_depths, label_segs, label_imgs, c2w, intrins, max_ray_nums=0, time_ids=None, dynamic_class=None, balance_weight=None, weight_adj=0.3, weight_dyn=0.0, use_wrs=True):
    r"""NuScenes Dataset.
    ...
    Args:
        ...
        label_imgs (Nx3): color GT of pixels
        c2w : camera to ego
    ...
    for time_id in time_ids:    # multi frames
        for i in time_ids[time_id]: # multi cameras of single frame
            ray = pts2ray(coors[i], label_depths[i], label_segs[i], label_imgs[i], c2w[i], intrins[i])
            rays.append(ray)
            ids.append(time_id)
...
def pts2ray_dense(coor, label_img, c2w, cam_intrinsic):
    rays_o, rays_d, viewdirs = get_rays(coor[:,0]+0.5, coor[:,1]+0.5, K=cam_intrinsic,c2w=c2w)
    return torch.cat([
        coor, # 0-1,
        rays_o, rays_d, viewdirs,  # 2:5,5:8,8:11
        label_img,  # 11:14
        ], dim=1
    )


def generate_rays_dense(coors, label_imgs, c2w, intrins, max_ray_nums=0, time_ids=None):
    r"""NuScenes Dataset.
    ...
    for time_id in time_ids:    # multi frames
        for i in time_ids[time_id]: # multi cameras of single frame
            ray = pts2ray_dense(coors[i], label_imgs[i], c2w[i], intrins[i])
            rays.append(ray)
            ids.append(time_id)

    rays = torch.cat(rays, dim=0)
    if max_ray_nums!=0 and rays.shape[0]>max_ray_nums:
        sampler = RandomSampler(range(rays.shape[0]), replacement=False)
        rays = rays[list(sampler)[:max_ray_nums]]

    return rays


def pts2ray_dense_lightwheel(coor, label_depth, label_img, c2w, cam_intrinsic):
    rays_o, rays_d, viewdirs = get_rays(coor[:,0]+0.5, coor[:,1]+0.5, K=cam_intrinsic,c2w=c2w)
    return torch.cat([
        coor,  # 0-1,
        rays_o, rays_d, viewdirs,  # 2:5,5:8,8:11
        label_img,  # 11:14
        label_depth.unsqueeze(1)  # 14
        ], dim=1
    )


def generate_rays_dense_lightwheel(coors, label_depths, label_imgs, c2w, intrins, max_ray_nums=0, time_ids=None):
    # generate rays
    rays = []
    ids = []
    for time_id in time_ids:    # multi frames
        for i in time_ids[time_id]: # multi cameras of single frame
            ray = pts2ray_dense_lightwheel(coors[i], label_depths[i], label_imgs[i], c2w[i], intrins[i])
            rays.append(ray)
            ids.append(time_id)
