...
def lovasz_softmax(probas, labels, classes='present', per_image=False, ignore=None, camera_mask=None):
    """
    ...
    if per_image:
        ...
        with autocast(False):
            loss = lovasz_softmax_flat(*flatten_probas(probas, labels, ignore, camera_mask=camera_mask), classes=classes)
    return loss
...
def flatten_probas(probas, labels, ignore=None, camera_mask=None):
    """
    ...
    if camera_mask is not None:
        ...
        valid = torch.logical_and(valid, camera_mask)
