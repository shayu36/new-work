...
from .utils import Raw2Alpha, Alphas2Weights, ub360_utils_cuda, silog_loss, l1_loss, cos_sim_loss
...
class NerfHead(nn.Module):
    ...
    def render_one_scene(self,
        ...
        color = color.permute(3,0,1,2).unsqueeze(0)
        color = F.grid_sample(color, ind_norm, mode='bilinear', align_corners=True)
        color = color.reshape(3, -1).T.reshape(*shape, 3)
    ...
    def compute_loss(self, results):
        ...
        target_color = results['target_color']
        color = results['render_color']
        color_loss = self.color_loss(color, target_color)
        losses['loss_render_color'] = color_loss * self.weight_color
    ...
    def compute_loss_temporal(self, results, interval):
        losses = {}
        if self.use_depth_sup:
            depth_loss = self.depth_loss(results['render_depth']+1e-7, results['target_depth'])
            losses[f'loss_render_depth_{int(interval)}s'] = depth_loss * self.weight_depth
        ...
        semantic_loss = criterion(semantic, target_semantic.long())
        losses[f'loss_render_semantic_{int(interval)}s'] = semantic_loss * self.weight_semantic

        target_color = results['target_color']
        color = results['render_color']
        color_loss = self.color_loss(color, target_color)
        losses[f'loss_render_color_{int(interval)}s'] = color_loss * self.weight_color

        if self.weight_entropy_last > 0:
            pout = results['alphainv_last'].clamp(1e-6, 1-1e-6)
            entropy_last_loss = -(pout*torch.log(pout) + (1-pout)*torch.log(1-pout)).mean()
            losses[f'loss_sdf_entropy_{int(interval)}s'] = self.weight_entropy_last * entropy_last_loss

        if self.weight_distortion > 0:
            n_max = len(results['t'])
            loss_distortion = flatten_eff_distloss(results['weights'], results['s'], 1/n_max, results['ray_id'])
            losses[f'loss_sdf_distortion_{int(interval)}s'] =  self.weight_distortion * loss_distortion
        return losses
    ...
    def render_color(self, results):
        color = segment_coo(
                        src=(results['weights'].unsqueeze(-1) * results['color']),
                        index=results['ray_id'],
                        out=torch.zeros([results['N_ray'], 3]).to(results['weights'].device),
                        reduce='sum')
        return color
    ...
    def forward(self, density, semantic, color, if_pretrain=False, if_temporal=False, dataset_type='Nuscenes', rays=None, bda=None, interval=0, **kwargs):
        if dataset_type == 'Nuscenes':
            gt_depths = rays[..., 2]
            gt_semantics = rays[..., 3]
            ray_o = rays[..., 4:7]
            ray_d = rays[..., 7:10]
            gt_colors = rays[..., 13:16]
            ...
            for batch_id in range(rays.shape[0]): 
                ...
                results['render_color'] = self.render_color(results)
                # compute loss
                if if_temporal:
                    loss_single = self.compute_loss_temporal(results, interval)
                else:
            ...
            for key in losses:
                losses[key] = losses[key] / semantic.shape[0]
