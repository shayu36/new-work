...
class OPUS(MVXTwoStageDetector):
    def __init__(self,
        ...
        self.num_query = pts_bbox_head.num_query
        self.num_fu_query = pts_bbox_head.num_fu_query
        self.num_fu_frames = pts_bbox_head.num_fu_frames
        self.num_fu_query_all = sum(self.num_fu_query)
        self.memory = {}
    ...
    def forward_train(self,
        ...
        if self.pts_bbox_head.num_classes!=17:
            voxel_semantics = torch.clamp(voxel_semantics - 10,max=self.pts_bbox_head.num_classes,min=0)
    ...
    def forward_test(self, img_metas, img=None, **kwargs):
        ...
        if False:
            ...
            from loaders.ray_metrics import calc_metrics
            occ_pred,_ = sparse2dense(result[0]['occ_loc'],result[0]['sem_pred'],(200,200,16),17)
            occ_file = os.path.join('../data/nuscenes/gts/',img_metas[0][0]['scene_name'],img_metas[0][0]['sample_idx'],'labels.npz')
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        ...
        pred_dict = dict(cls_scores = outs['all_cls_scores'][-1],
                         refine_pts = outs['all_refine_pts'][-1])
        result = self.pts_bbox_head.get_occ(pred_dict)
        outs['occ_pred'] = result
        return outs
    ...
    def simple_test_online(self, img_metas, img=None, rescale=False):
        ...
        torch.cuda.synchronize()
        t1 = time.time()
        # print('img_backbone time: ',t1-t0,end=' ')
        ...
        result = self.simple_test_pts(img_feats, img_metas, rescale=rescale)
        t2 = time.time()
        # print('RAP: ', t2 - t1,end=' ')
        return result